...
with DAG(
    dag_id='import_demo',
    description='Import initial data of 3 tickers to Postgresql',
    start_date=datetime(2023, 1, 1),
    schedule_interval=None,
    tags=["init"]
) as dag:
    path = os.getcwd()
    name = "data_to_db_demo.py"
    ...
    listing_companies = SparkSubmitOperator(task_id = "listing_companies",
                                    ...
                                    application_args = ['--table', 'listing_companies', '--init', "True", '--demo', 'True'], 
                                    dag = dag)
    ...
    today_stock_history = SparkSubmitOperator(task_id = "today_stock_history",
                                    application = app_path,
                                    jars = driver_path,
                                    num_executors = 4,
                                    executor_cores = 4,     
                                    conn_id = "spark_default",      
                                    application_args = ['--table', 'stock_history', '--start_date', get_date('yesterday'), '--end_date', get_date('today'), '--demo', 'True'], 
                                    dag = dag)
    
    today_stock_history_calculation = SparkSubmitOperator(task_id = "today_stock_history_calculation",
                                    application = app_path,
                                    jars = driver_path,
                                    num_executors = 4,
                                    executor_cores = 4,
                                    conn_id = "spark_default",
                                    application_args = ['--calculation', 'True','--demo', 'True'], 
                                    dag = dag)
    ...
    >> [business_model_rating,balance_sheet, stock_history_calculation] >> today_stock_history >>  [today_stock_history_calculation, stock_intraday_transaction]  #stock history is heavy